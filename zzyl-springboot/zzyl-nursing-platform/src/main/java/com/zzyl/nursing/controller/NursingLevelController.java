...
public class NursingLevelController extends BaseController
{
     ...
     * 查询护理等级列表
     */
    @PreAuthorize("@ss.hasPermi('zzyl:level:list')")
    @GetMapping("/list")
     ...
     * 获取所有启用的护理计划（下拉框用）
     */
    @GetMapping("/getEnablePlanList")
    public AjaxResult getEnablePlanList()
    {
        List<NursingPlan> planList = nursingPlanService.selectAllEnablePlan();
     ...
     * 导出护理等级列表
     */
    @PreAuthorize("@ss.hasPermi('zzyl:level:export')")
     ...
     * 获取护理等级详细信息
     */
    @PreAuthorize("@ss.hasPermi('zzyl:level:query')")
    @GetMapping(value = "/{id}")
     ...
     * 新增护理等级
     */
    @PreAuthorize("@ss.hasPermi('zzyl:level:add')")
     ...
     * 修改护理等级
     */
    @PreAuthorize("@ss.hasPermi('zzyl:level:edit')")
     ...
     * 删除护理等级
     */
    @PreAuthorize("@ss.hasPermi('zzyl:level:remove')")
